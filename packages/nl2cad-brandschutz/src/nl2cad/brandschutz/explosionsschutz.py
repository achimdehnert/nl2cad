"""
nl2cad.brandschutz.explosionsschutz
=====================================
ExplosionsschutzDokument (ESD) nach BetrSichV § 6 Abs. 9.

Implementierungsreihenfolge: Schritt 3.
...
Invarianten (ADR B-05):
  ...
  - ex_bereiche darf nicht leer sein wenn ESD ausgefuellt
...
class BeurteilungsStatus(StrEnum):
    ...
    NICHT_BEURTEILBAR: Datenbasis ungenuegend fuer Beurteilung.
    BESTAETIGT:       Durch befugten Sachverstaendigen bestaetigt (ausserhalb nl2cad).
...
class ExplosionsschutzMassnahme:
    ...
    prioritaet: int = 1  # 1=primaer, 2=sekundaer, 3=tertiaer
...
class ExplosionsschutzDokument:
    ...
    Invarianten (werden in __post_init__ geprueft):
    - betrieb_name: nicht leer
    - erstellungsdatum: gueltig, nicht in der Zukunft
    - naechste_pruefung: nach erstellungsdatum
    - ex_bereiche: mindestens 1 Eintrag wenn Dokument erstellt wird

    Alle Pflichtfelder gemaess ESD_PFLICHTFELDER aus constants.py.
    """
    ...
    def __post_init__(self) -> None:
        """Invarianten-Pruefung — kein stilles leeres ESD (ADR B-05)."""
        errors: list[str] = []
        ...
        if self.erstellungsdatum:
            try:
                ...
                if erstellt > date.today():
                    errors.append(
                        f"erstellungsdatum {self.erstellungsdatum} liegt in der Zukunft"
                    )
            except ValueError:
                errors.append(
                    f"erstellungsdatum '{self.erstellungsdatum}' ist kein gueltiges ISO-Datum"
                )
            ...
            try:
                ...
                if pruefung <= erstellt:
                    errors.append(
                        ...
                        f"muss nach erstellungsdatum {self.erstellungsdatum} liegen"
                    )
            except ValueError:
        ...
        if errors:
            raise ValueError(
                f"ExplosionsschutzDokument Invarianten verletzt: {'; '.join(errors)}"
            )
    ...
    def ist_vollstaendig(self) -> bool:
        """Prueft ob alle BetrSichV-Pflichtfelder ausgefuellt sind."""
    ...
    def geraetekategorie_fuer_zone(self, zone: ExZone) -> str:
        """Gibt ATEX-Geraetekategorie fuer eine Zone zurueck."""
