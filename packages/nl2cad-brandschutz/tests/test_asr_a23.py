...
"""
import pytest
from nl2cad.brandschutz.models import BrandschutzAnalyse, Fluchtweg, MängelSchwere
...
class TestASRA23Validator:
    ...
    def test_zu_langer_fluchtweg(self):
        ...
        result = self.validator.validate(analyse)
        kritisch = [m for m in result.maengel if m.schwere == MängelSchwere.KRITISCH]
    ...
    def test_grenzwert_fluchtweg_35m_warnung(self):
        ...
        result = self.validator.validate(analyse)
        warnungen = [m for m in result.maengel if m.schwere == MängelSchwere.WARNUNG]
    ...
    def test_keine_fluchtwege_warnung(self):
        ...
        assert any("Keine Fluchtwege" in m.beschreibung for m in result.maengel)

    def test_laenge_nicht_pruefbar(self):
        analyse = BrandschutzAnalyse()
        analyse.fluchtwege.append(Fluchtweg(name="FW", laenge_m=0.0, hat_notausgang=True))
        self.validator.validate(analyse)
        assert analyse.fluchtwege[0].laenge_ok is None

    def test_to_dict(self):
        analyse = BrandschutzAnalyse()
        analyse.fluchtwege.append(Fluchtweg(name="FW", laenge_m=25.0, breite_m=1.2, hat_notausgang=True))
        self.validator.validate(analyse)
        d = analyse.to_dict()
...
class TestIntentClassifier:
    ...
    def test_import(self):
        ...
        result = clf.classify("Zeige mir alle Räume und ihre Flächen")
        assert result.intent == NLIntent.RAUMANALYSE
        assert result.confidence > 0

    def test_din277_intent(self):
        from nl2cad.nlp.intent import IntentClassifier, NLIntent
        clf = IntentClassifier()
        result = clf.classify("Berechne die DIN 277 Nutzungsarten")
        assert result.intent == NLIntent.DIN277

    def test_gaeb_intent(self):
        from nl2cad.nlp.intent import IntentClassifier, NLIntent
        clf = IntentClassifier()
        result = clf.classify("Erstelle das GAEB Leistungsverzeichnis")
        assert result.intent == NLIntent.GAEB_EXPORT

    def test_unbekannt(self):
        from nl2cad.nlp.intent import IntentClassifier, NLIntent
        clf = IntentClassifier()
        result = clf.classify("Wie ist das Wetter heute?")
        assert result.intent == NLIntent.UNBEKANNT
